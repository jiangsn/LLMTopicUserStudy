...
export function NextButton({
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if ((event.key === 'Enter' || event.key === 'ArrowRight')
        && !isEditing && !disabled && !isNextDisabled && buttonTimerSatisfied) {
  ...
  const hidePreviousWhenPreviousComponent = config?.meta?.hidePreviousWhenPreviousComponent;
  const previousComponent = typeof currentStep === 'number'
    ? participantSequence[currentStep - 1]
    : undefined;
  const showPreviousButton = Boolean(config?.previousButton
    && (typeof hidePreviousWhenPreviousComponent !== 'string'
      || hidePreviousWhenPreviousComponent !== previousComponent));
  return (
    <>
      <Group justify={showPreviousButton ? 'space-between' : 'right'} gap="xs" mt="sm">
        {showPreviousButton && (
          <PreviousButton
