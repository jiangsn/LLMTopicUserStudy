...
import { getMissingProlificParams } from '../utils/prolificParams';
...
export function Shell({ globalConfig }: { globalConfig: GlobalConfig }) {
  ...
  const [prolificEntryError, setProlificEntryError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function initializeUserStoreRouting() {
      ...
      setProlificEntryError(null);

      if (import.meta.env.VITE_REQUIRE_PROLIFIC_PARAMS === 'true') {
        const missingParams = getMissingProlificParams(searchParams);
        if (missingParams.length > 0) {
          setProlificEntryError(
            'This recruitment link is incomplete. Please return to Prolific and open the study from your submission page.',
          );
  ...
  } else if (prolificEntryError) {
    content = (
      <Stack align="center" gap="sm" maw={620} mx="auto" mt="15vh" px="md">
        <Title order={2}>Prolific link required</Title>
        <Text ta="center">{prolificEntryError}</Text>
        <Text c="dimmed" size="sm" ta="center">
          No study response has been created from this page.
        </Text>
      </Stack>
    );
