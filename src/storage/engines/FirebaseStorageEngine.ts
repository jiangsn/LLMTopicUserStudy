...
export class FirebaseStorageEngine extends CloudStorageEngine {
  ...
  /**
   * Cloud studies use the anonymous Firebase Auth UID as the internal participant ID.
   * Recruitment identifiers such as PROLIFIC_PID remain ordinary answer fields and
   * never become storage object names or Firestore document IDs.
   */
  override async getCurrentParticipantId(_urlParticipantId?: string) {
    const ready = await this.initializeAnonymousAuth();
    const auth = getAuth();
    const authUid = ready ? auth.currentUser?.uid : undefined;
    if (!authUid) {
      throw new Error('FirebaseAuthError: Anonymous participant identity is unavailable.');
    }

    this.currentParticipantId = authUid;
    if (this.studyId) {
      const storageKey = `${this.collectionPrefix}${this.studyId}/currentParticipantId`;
      await this.participantStore.setItem(storageKey, authUid);
    }
    return authUid;
  }

  override async peekCurrentParticipantId(studyId?: string) {
    const authUid = getAuth().currentUser?.uid;
    if (authUid) return authUid;
    return super.peekCurrentParticipantId(studyId);
  }
